...
serve(async (req) => {
  try {
    ...
    // Get all active (non-converted) carts with items
    ...
    for (const row of reminders) {
      ...
      if (msSinceLast < delayMs) { skipped++; continue; }
...
<div style="max-width:600px;margin:0 auto;background:#fff">
  ...
  <div style="background:#111;padding:24px 32px;display:flex;align-items:center;gap:12px">
    <span style="font-family:'Bebas Neue',Arial,sans-serif;font-size:1.8rem;letter-spacing:.08em;color:#fff">Big<span style="color:#B8892A">Boy</span>Peps</span>
  </div>

  <div style="padding:32px">
    <p style="font-family:Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:#B8892A;margin:0 0 8px">Your Cart</p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #eee;margin-bottom:24px">
      ...
      <tfoot>
        <tr>
          <td colspan="2" style="padding:10px 16px;font-size:13px;font-weight:700;color:#111">Total</td>
          <td style="padding:10px 16px;font-size:16px;font-weight:900;color:#B8892A;text-align:right">$${total.toFixed(2)}</td>
        </tr>
      </tfoot>
    </table>

    <div style="text-align:center;margin-bottom:24px">
      <a href="${SHOP_URL}" style="display:inline-block;background:#B8892A;color:#fff;font-size:12px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;padding:14px 32px;text-decoration:none;border-radius:3px">
        Complete My Order →
      </a>
    </div>
    ...
    <p style="font-size:11px;color:#aaa;text-align:center;line-height:1.6">
      ...
      <a href="https://bigboypeps.com/index.html?unsubscribe=${row.user_id}" style="color:#bbb">Unsubscribe from cart reminders</a>
    </p>
  </div>
